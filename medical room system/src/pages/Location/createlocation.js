...
const Createlocation = ({ func }) => {
  ...
  const checkDuplicateID = async (id) => {
    const response = await fetch(`http://localhost:8090/api/location/checkDuplicateID/` + id, {
      method: 'GET',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <Modal.Body className="col-11  mx-auto" closeButton>
                <div className="row" style={{ marginTop: '-20px' }}>
                  <div className="col-7">
                    <h3 className="mb-5">Add Medical-Room</h3>
                  </div>
                </div>
                <div className="row">
                  ...
                  <Col>
                    <Form.Group className="form-group">
                      <label for="exampleSelect">Location:</label>  <label className="form-label textmark"> *</label>
                      <Input
                        type="select"
                        value={Local}
